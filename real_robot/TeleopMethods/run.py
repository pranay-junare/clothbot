...
def ros_update(fields, ros_data, control_modes, URs, pubs):
        ...
        if control_modes[arm] == 'Spark':
            if arm.lower()+'_spark_angle' in ros_data:
                ...
                if ros_data[arm.lower() + '_change_mode'] == True:
                    ...
                    dq = [a-u+h for a, u, h in zip(angles, URs.get_Q(arm), homes[arm])]
                    # print("Diff: ", dq)
                ...
                # print(angles)
                if arm == "Lightning":
                    gripper = np.clip((0-angles[6]*2), 0, 1)
                else:
                    gripper = np.clip((1.1-angles[6]*2), 0, 1)
                ...
                if "thunder_spark_enable" in ros_data:
                    global spark_enable
                    old_spark_enable = spark_enable[arm] if arm in spark_enable else False
                    spark_enable[arm] = ros_data["thunder_spark_enable"]
