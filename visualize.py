...
def summarize(path):
    ...
    output_dir = os.path.join(os.path.dirname(path), 'plots')
    ...
    sns.despine()
    plt.title('Best Coverage over Episodes')
    plt.grid()
    file_path = os.path.join(output_dir, 'best_coverage_plot.png')
    ...
    sns.lineplot(data=df, y='Final Coverage', x='Episodes')
    sns.despine()
    plt.title('Final Coverage over Episodes')
    plt.grid()
    file_path = os.path.join(output_dir, 'final_coverage_plot.png')
    plt.savefig(file_path, dpi=600, bbox_inches='tight')
    plt.show()
    ...
    averaged_coverages = []
    episodes = []
    init_coverage = stats['init_coverage/hard/distribution']
    for i in range(len(stats['init_coverage/hard/distribution'])):
        if i < window:
            continue
        for j in range(i-window, i+1):
            if j > 0:
                averaged_coverages.append(init_coverage[j])
                episodes.append(i)
    df['Initial Coverage'] = averaged_coverages
    df['Episodes'] = episodes
    sns.lineplot(data=df, y='Initial Coverage', x='Episodes')
    sns.despine()
    plt.title('Initial Coverage over Episodes')
    plt.grid()
    file_path = os.path.join(output_dir, 'init_coverage_plot.png')
    plt.savefig(file_path, dpi=600, bbox_inches='tight')
    plt.show()    
    ...
    plt.grid()
    file_path = os.path.join(output_dir, 'delta_coverage_plot.png')
    plt.savefig(file_path, dpi=600, bbox_inches='tight')
    plt.show()
    ...
    plt.grid()
    file_path = os.path.join(output_dir, 'postaction_coverage_plot.png')
    plt.savefig(file_path, dpi=600, bbox_inches='tight')
    plt.show()
    ...
    plt.grid()
    file_path = os.path.join(output_dir, 'action_primitive.png')
    plt.savefig(file_path, dpi=600, bbox_inches='tight')
    plt.show()
